# handlers/invitation.py
import re
from telegram import Update, ChatMember, ChatMemberUpdated
from telegram.ext import ContextTypes
from telegram.error import TelegramError
from database import Session, User
from models.referral import Referral
from services import economy

# Config
INVITE_REWARD_POINTS = 100

async def generate_invite_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Command: 专属链接
    Generates a link named "Ref:USER_ID" so we can track it later.
    """
    chat = update.effective_chat
    user = update.effective_user

    if chat.type == 'private':
        await update.message.reply_text("⚠️ 请在群组中使用此命令。")
        return

    try:
        # LOGIC FIX: We put the ID strictly at the start of the name for easy parsing
        # Format: "Ref:123456789 | Name"
        link_name = f"Ref:{user.id} | {user.first_name}"
        
        # Invite links allow a limited name length, so we truncate if needed
        if len(link_name) > 32:
            link_name = link_name[:32]

        invite_link = await context.bot.create_chat_invite_link(
            chat_id=chat.id,
            name=link_name, 
            creates_join_request=False
        )
        
        await update.message.reply_text(
            f"✅ {user.mention_html()} 的专属链接:\n\n"
            f"{invite_link.invite_link}\n\n"
            f"🎉 邀请新用户加入，每位奖励 {INVITE_REWARD_POINTS} 积分!",
            parse_mode='HTML'
        )
        
    except TelegramError as e:
        await update.message.reply_text("❌ 生成失败: 机器人不是管理员或没有 '管理邀请链接' 权限。")
        print(f"Invite Link Error: {e}")

async def track_join_event(update: Update, context: ContextTypes.DEFAULT_TYPE):
    print(f"🔎 Checking join event in chat {update.effective_chat.id}...")

    result = _extract_status_change(update.chat_member)
    if result is None:
        return

    was_member, is_member = result
    
    # Only track NEW joins
    if was_member or not is_member:
        return

    invite_link = update.chat_member.invite_link
    new_member = update.chat_member.new_chat_member
    
    # 1. Check if an invite link was used
    if not invite_link:
        print("❌ No invite link used.")
        return

    # 2. LOGIC FIX: Parse the Inviter ID from the link name
    # We look for "Ref:12345" in the link name
    # The 'creator' field is the Bot, so we IGNORE it.
    link_name = invite_link.name or ""
    
    # Regex to find "Ref:" followed by numbers
    match = re.search(r"Ref:(\d+)", link_name)
    
    if not match:
        print(f"❌ Link name '{link_name}' does not contain a User ID.")
        return

    inviter_id = int(match.group(1))
    joined_user = new_member.user

    # Prevent self-invite
    if inviter_id == joined_user.id:
        return

    print(f"✅ Invite Validated! Real Inviter: {inviter_id}, New User: {joined_user.id}")

    session = Session()
    try:
        # 3. Check for duplicates in DB
        exists = session.query(Referral).filter_by(
            inviter_id=inviter_id, 
            invited_user_id=joined_user.id
        ).first()

        if exists:
            print("⚠️ Referral already exists in DB.")
            return
        
        # 4. Save to DB
        new_ref = Referral(inviter_id=inviter_id, invited_user_id=joined_user.id)
        session.add(new_ref)
        session.commit()
        
        # 5. Award Points
        # Retrieve inviter info (might need to fetch from chat or DB if they aren't in `user` object here)
        # We try to get user from DB, if not exists, we can't get their name easily, but we can add points.
        economy.add_points(inviter_id, float(INVITE_REWARD_POINTS))

        # Notify Group
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text=f"📢 邀请成功!\n"
                 f"🎉 用户 `{inviter_id}` 邀请了 {joined_user.mention_html()}!\n"
                 f"💰 获得奖励: {INVITE_REWARD_POINTS} 积分",
            parse_mode='HTML'
        )

    except Exception as e:
        print(f"❌ Referral Error: {e}")
        session.rollback()
    finally:
        session.close()

def _extract_status_change(chat_member_update: ChatMemberUpdated):
    """
    Helper to determine if the user joined or left.
    """
    status_change = chat_member_update.difference().get("status")
    old_is_member, new_is_member = chat_member_update.difference().get("is_member", (None, None))

    if status_change is None:
        return None

    old_status, new_status = status_change
    
    was_member = old_status in [
        ChatMember.MEMBER,
        ChatMember.OWNER,
        ChatMember.ADMINISTRATOR,
    ] or (old_status == ChatMember.RESTRICTED and old_is_member is True)

    is_member = new_status in [
        ChatMember.MEMBER,
        ChatMember.OWNER,
        ChatMember.ADMINISTRATOR,
    ] or (new_status == ChatMember.RESTRICTED and new_is_member is True)

    return was_member, is_member